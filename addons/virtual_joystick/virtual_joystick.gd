...
@export var action_left := "move left"
@export var action_right := "move right"
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if use_input_actions:
		...
		# Press actions
		if output.x < 0:
			Input.action_press(action_left, -output.x/abs(output.x))
		if output.x > 0:
			Input.action_press(action_right, output.x/abs(output.x))
		if output.y < 0:
